"""Kilo Gateway model fetching.

Fetches available models from Kilo Gateway API.
"""

import httpx

from app.providers.kilo_gateway.config import KiloGatewayConfig
from app.utils.url import url_path_join

_config = KiloGatewayConfig()
...
def parse_response(data: dict) -> list:
    """Extract models list from Kilo Gateway API response."""
    return data.get("data", [])


async def fetch_models(api_key: str) -> list[dict]:
    """Fetch available models from Kilo Gateway.
    ...
    Raises:
        ...
        httpx.ConnectError: If cannot connect to Kilo Gateway.
