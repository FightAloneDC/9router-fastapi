"""Minimax (China) model fetching.

Fetches available models from Minimax (China) API.
"""

import httpx

from app.providers.minimax_cn.config import MinimaxCnConfig
from app.utils.url import url_path_join

_config = MinimaxCnConfig()
...
def parse_response(data: dict) -> list:
    """Extract models list from Minimax (China) API response."""
    return data.get("data", [])


async def fetch_models(api_key: str) -> list[dict]:
    """Fetch available models from Minimax (China).

    Args:
        api_key: Minimax (China) API key.
    ...
    Raises:
        ...
        httpx.ConnectError: If cannot connect to Minimax (China).
